...
def ensure_dirs() -> None:
    for path in (RAW_DIR, PROCESSED_DIR, TABLE_DIR, FIGURE_DIR, TABLEAU_DIR, MEMO_DIR):
...
def write_inventory_and_notes(sales: pd.DataFrame, rent: pd.DataFrame) -> None:
    ...
    notes = [
        ...
        "## Tableau CSV",
        "",
        "- `residential/data/processed/tableau/01_admin_dong_demand.csv`: ADM_CD가 포함된 행정동 단위 수요/인구 테이블입니다.",
        "- `residential/data/processed/tableau/02_legal_dong_cost_summary.csv`: 법정동 x 건축유형 단위 매매/전세/월세 요약입니다.",
        "- `residential/data/processed/tableau/03_admin_dong_cost_proxy.csv`: 행정동 지도에 가격을 얹기 위한 추정 결합 테이블입니다.",
        "- `residential/data/processed/tableau/04_gu_housing_stock_reference.csv`: 구 단위 주택 재고 wide table입니다.",
        "- `residential/data/processed/tableau/05_gu_demand_and_stock_reference.csv`: 구 단위 수요와 주택 재고를 함께 본 reference table입니다.",
        "- `residential/data/processed/tableau/transactions_sales_long.csv`, `transactions_rent_long.csv`: Tableau에서 직접 필터링할 수 있는 실거래 long table입니다.",
        "",
        "## 추가로 있으면 좋은 데이터",
        "",
    ...
    (MEMO_DIR / "residential_data_notes.md").write_text("\n".join(notes), encoding="utf-8")
    ...
    tableau_readme = [
        ...
        "",
        "1. 지도 레이어: `residential/data/raw/Dataset_v1/seongnam_dong.geojson`을 Tableau의 spatial file로 불러옵니다.",
